		...
		"paths": map[string]any{
							...
							"multipart/form-data": map[string]any{
								"schema": map[string]any{
									...
									"properties": map[string]any{
										...
										"notes":            map[string]any{"type": "string", "description": "Optional JSON-encoded array of positional note boxes ({x, y, w, h, body}) for the pushed source."},
							...
							"application/json": map[string]any{
								"schema": map[string]any{
									...
									"properties": map[string]any{
										...
										"md5":              map[string]any{"type": "string", "description": "Optional md5 the source claims for the file (<=64 chars). Recorded on the origin row as an audit trail; never a dedup key."},
										"commentary":       map[string]any{"type": "string", "description": "Optional artist commentary for the pushed source (<=10000 chars)."},
										"notes":            map[string]any{"type": "array", "items": map[string]any{"type": "object"}, "description": "Optional positional note boxes ({x, y, w, h, body}) for the pushed source."},
										"collection":       map[string]any{"type": "string", "description": "Optional collection label (images.series). Written on the new row."},
			...
			"/images/{id}/enrich": map[string]any{
				"post": map[string]any{
					...
					"description": "Metadata-only counterpart of a duplicate push, used by monloader's source refetch. Shares the duplicate-push merge semantics: the origin upserts, the source's own tag slice reconciles to the pushed set, and empty url / md5 / commentary / notes leave the stored values be.",
					"parameters":  []map[string]any{pathParam("id", "Image ID"), galleryParam()},
					"requestBody": jsonBodySchema(true, map[string]any{
						"type": "object",
						"properties": map[string]any{
							...
							"verify":     map[string]any{"type": "boolean", "description": "When true and source_md5 is non-empty, the stored file is hashed and compared first; a mismatch applies nothing and returns 409."},
							"commentary": map[string]any{"type": "string"},
							"notes":      map[string]any{"type": "array", "items": map[string]any{"type": "object"}, "description": "Positional note boxes ({x, y, w, h, body}); a non-empty array replaces the set this origin contributed."},
						},
					}),
					"responses": map[string]any{
						"200": resp("Metadata applied", "#/components/schemas/EnrichResponse"),
						"404": resp("Not found", "#/components/schemas/Error"),
						"409": resp("Hash mismatch: the source no longer serves this file; nothing was changed", "#/components/schemas/Error"),
